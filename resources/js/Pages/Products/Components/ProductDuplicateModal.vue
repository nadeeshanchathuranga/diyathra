        ...
        <div class="mb-6 bg-white rounded-xl p-4 border border-gray-200">
          ...
          <div class="grid grid-cols-1 gap-4 md:grid-cols-2 lg:grid-cols-3">
            ...
            <div>
              <label class="block mb-2 text-sm font-medium text-gray-700"
                >Purchase Price
                <span v-if="form.sales_unit_id" class="text-green-500">
              ...
              <input
                ...
                placeholder="0.00"
              />
              <span class="text-xs text-gray-400">{{ purchasePriceFormulaText }}</span>
            </div>
...
const salesUnitsPerPurchaseUnit = computed(() => {
  const purchaseToTransferRate = Number(form.purchase_to_transfer_rate) || 0;
  const transferToSalesRate = Number(form.transfer_to_sales_rate) || 0;
...
const purchasePricePerSalesUnitInput = computed({
  ...
  set(value) {
    ...
    const conversionRate = salesUnitsPerPurchaseUnit.value;
    const purchaseUnitPrice = conversionRate > 0 ? numericValue * conversionRate : numericValue;

    form.purchase_price = Number(purchaseUnitPrice.toFixed(2));
  },
});

const purchasePriceFormulaText = computed(() => {
  const purchaseUnitName = getPurchaseUnitName(form.purchase_unit_id) || "Purchase Unit";
  const salesUnitName = getSalesUnitName(form.sales_unit_id) || "Sales Unit";
  const conversionRate = salesUnitsPerPurchaseUnit.value;

  if (conversionRate <= 0) {
    return "Set conversion rates to auto-calculate from purchase unit";
  }
