...
class BillSettingController extends Controller
{
    ...
    {
        // Always use the most recently created settings row for consistency
        // with modules that already read BillSetting::latest('id')->first().
        $setting = BillSetting::latest('id')->first();
    ...
    public function store(Request $request)
    {
        ...
        // Update the latest settings record if present; otherwise create new.
        $setting = BillSetting::latest('id')->first() ?? new BillSetting();
        ...
        if ($request->hasFile('logo')) {
            ...
            if ($srcImg) {
                ...
                imagefilledrectangle($newImg, 0, 0, $newW - 1, $newH - 1, $transparent);
                ...
                // Use actual image dimensions for safe pixel iteration
                $actualW = imagesx($newImg);
                $actualH = imagesy($newImg);
        ...
        return redirect()->back()->with('success', 'Bill settings updated successfully.');
            ...
            return redirect()->back()->with('error', 'Failed to update bill settings: ' . $e->getMessage());
        }
    }
}
